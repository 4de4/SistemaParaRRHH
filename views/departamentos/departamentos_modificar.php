...
<head>
    ...
    <style>
        /* Estilos similares al formulario de nuevo o personaliza */
        ...
        .btn-submit { padding: 10px 20px; background-color: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 1em; }
...
<body>
    <div class="container">
        ...
        <?php
        $depto = $data['departamento_data'] ?? [];
        $id_d = $depto['id_d'] ?? '';
        $nombre = $depto['nombre'] ?? '';
        $ubicacion = $depto['ubicacion'] ?? '';
        ?>

        <form id="modificarDepartamentoForm" method="POST" action="crud.php?c=departamento&a=actualizar" autocomplete="off">
            <input type="hidden" name="id_d" value="<?php echo htmlspecialchars($id_d); ?>">

            <div class="form-group">
                <label for="nombre">Nombre del Departamento:</label>
                <input type="text" id="nombre" name="nombre" value="<?php echo htmlspecialchars($nombre); ?>" required maxlength="60">
            </div>

            <div class="form-group">
                <label for="ubicacion">Ubicación:</label>
                <input type="text" id="ubicacion" name="ubicacion" value="<?php echo htmlspecialchars($ubicacion); ?>" required maxlength="100">
            </div>

            <div class="form-group" style="text-align: center;">
                <button type="submit" name="actualizar" class="btn-submit">Actualizar Departamento</button>
